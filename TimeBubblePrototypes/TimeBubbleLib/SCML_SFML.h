...
class FileSystem : public SCML::FileSystem
{
	...
	SCML_MAP(SCML_PAIR(int, int), SCML_PAIR(float, float)) pivots;

	 void setImagePivot(int folderID, int fileID, float pivot_x, float pivot_y);
	 SCML_PAIR(float, float) getImagePivot(int folderID, int fileID) const;
...
class Entity : public SCML::Entity
{
	...
	virtual SCML_PAIR(float, float) getImagePivot(int folderID, int fileID) const;
