...
static void log(const char* formatted_text, ...)
{
    ...
    vsprintf_s(buffer, formatted_text, lst);
    va_end(lst);
...
bool Data::Entity::Animation::Mainline::Key::Object::load(TiXmlElement* elem)
{
    ...
    x = xmlGetFloatAttr(elem, "x", 0.0f);
    y = xmlGetFloatAttr(elem, "y", 0.0f);
    pivot_x = xmlGetFloatAttr(elem, "pivot_x", 0.0);
...
bool Data::Entity::Animation::Timeline::Key::Object::load(TiXmlElement* elem)
{
    ...
    x = xmlGetFloatAttr(elem, "x", 0.0f);
    y = xmlGetFloatAttr(elem, "y", 0.0f);
    pivot_x = xmlGetFloatAttr(elem, "pivot_x", -1.0f);
    pivot_y = xmlGetFloatAttr(elem, "pivot_y", -1.0f);
    angle = xmlGetFloatAttr(elem, "angle", 0.0f);
    w = xmlGetFloatAttr(elem, "w", 0.0f);
    h = xmlGetFloatAttr(elem, "h", 0.0f);
    ...
    SCML_BEGIN_MAP_FOREACH_CONST(data->folders, int, SCML::Data::Folder*, folder)
    {
        SCML_BEGIN_MAP_FOREACH_CONST(folder->files, int, SCML::Data::Folder::File*, file)
        {
            if(file->type == "image")
            {
				...
				setImagePivot(folder->id, file->id,file->pivot_x, file->pivot_y);
...
void Entity::draw_tweened_object(Animation::Mainline::Key::Object_Ref* ref)
{
    ...
    if(obj1 != NULL)
    {
		...
		float pX1 = obj1->pivot_x;
		float pX2 = obj2->pivot_x;
		
		if (pX1 == -1.0f) pX1 = getImagePivot(obj1->folder, obj1->file).first;
		if (pX2 == -1.0f) pX2 = getImagePivot(obj2->folder, obj2->file).first;

		float pY1 = obj1->pivot_y;
		float pY2 = obj2->pivot_y;
		
		if (pY1 == -1.0f) pY1 = getImagePivot(obj1->folder, obj1->file).second;
		if (pY2 == -1.0f) pY2 = getImagePivot(obj2->folder, obj2->file).second;

        float pivot_x_ratio = lerp(pX1, pX2, t);
        float pivot_y_ratio = lerp(pY1, pY2, t);
